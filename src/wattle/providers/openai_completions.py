...
The Chat Completions wire format is structurally different from Wattle's
internal block-based shape in three ways, all handled here:
  ...
  * Tool results live on a dedicated `"tool"` wire role; each
    `ToolResultBlock` in a Wattle user message is lifted to its own wire
    message (one-to-many).
...
def _message_to_wire(message: Message) -> list[dict[str, Any]]:
    """Translate one Wattle message into zero or more OpenAI wire messages.
