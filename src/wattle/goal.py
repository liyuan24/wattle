...
def build_goal_continuation_prompt(goal: GoalState) -> str:
    ...
    return "\n".join(
        [
            ...
            "\"complete\" and include concise evidence plus the structured fields ",
            "`requirements_evidence`, `interface_evidence`, `semantic_evidence`, ",
            "and `remaining_risk`. These fields must prove the current-state ",
            "verification covered the explicit requirements, exact downstream ",
            "interface or representation contract, semantic/domain correctness, ",
