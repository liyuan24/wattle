...
def build_goal_continuation_prompt(goal: GoalState) -> str:
    ...
    return "\n".join(
        [
            ...
            "  parse or invoke the final file exactly as the downstream consumer will, ",
            "  then derive any domain checks from that parsed artifact. Do not validate ",
            "  only the values, components, thresholds, or arguments you intended to ",
            "  write. A script that merely runs, writes a file, and produces requested ",
            "  fields is surface evidence until its output is checked against an oracle, ",
            "  source data, expected behavior, or a faithful domain invariant.",
