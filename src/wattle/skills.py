"""Discovery and explicit invocation helpers for Wattle skills.

Wattle discovers skills from ``~/.wattle`` and from ``.wattle`` directories
walking from the current working directory up through its parents. Skills use
the directory layout ``.wattle/skills/<skill-name>/SKILL.md``. Skill files may
...
def format_skills_for_system_prompt(skills: list[Skill]) -> str:
    ...
    if not skills:
        return "No Wattle skills discovered."

    lines = [
        "Available Wattle skills:",
...
def expand_skill_invocation(text: str, cwd: str | Path) -> str | None:
    ...
    return (
        f"Use the Wattle skill {skill.name!r} from {skill.path}.\n\n"
        "<skill>\n"
...
def _skill_roots(cwd: str | Path) -> list[tuple[str, Path]]:
    ...
    roots: list[tuple[str, Path]] = [("user", home / ".wattle")]
    seen = {(home / ".wattle").resolve()}
