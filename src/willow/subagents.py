...
from willow.message_history import monitor_event_text_blocks
from willow.permissions import PermissionGate
...
from willow.request_preparation import RequestPreparer, acomplete_with_recovery
from willow.tools.base import Tool, ToolSpec
...
class SubagentManager:
    ...
    Subagents run in Willow-managed threads and use the same provider/tool
    abstraction as the parent. They are not implemented by shelling out to a
    second Willow CLI process.
    """
    ...
    def _subagent_system(system: str | None, instructions: str | None) -> str | None:
        if not instructions:
            return system
        addition = (
            "You are running as a Willow managed subagent. Focus only on the "
